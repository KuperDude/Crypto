...
extension PortfolioView {
    ...
    private func getCurrentValue() -> Double {
        if let quantity = Double(quantityText.replaceCommaWithDot), let price = selectedCoin?.currentPrice {
    ...
    private var trailingNavBarButtons: some View {
        HStack(spacing: 10) {
            ...
            .opacity(
                selectedCoin != nil && selectedCoin?.currentHoldings != Double(quantityText.replaceCommaWithDot) ? 1.0 : 0.0
            )
        }
        .font(.headline)
    }
